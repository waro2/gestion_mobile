...
export class OperationService {
    ...
    protected convertDateFromClient(operation: IOperation): IOperation {
        const copy: IOperation = Object.assign({}, operation, {
            dateoperations:
                operation.dateoperations != null && operation.dateoperations.isValid() ? operation.dateoperations.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
        if (res.body) {
            res.body.dateoperations = res.body.dateoperations != null ? moment(res.body.dateoperations) : null;
        }
        return res;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((operation: IOperation) => {
                operation.dateoperations = operation.dateoperations != null ? moment(operation.dateoperations) : null;
            });
        }
        return res;
    }
